...
const bitz_ws = require("./bitz_ws");
const bitz = require("./bitz");

let ws_api = new bitz_ws(conf.bitz_ws_url);
...
async function cancelAllTrackedDestOrders() {
	...
	for (let source_price in assocDestOrdersBySourcePrice) {
		...
		bitz.createAndSendCancel(dest_order.hash);
	}
	await wait(1000);
}
...
async function cancelAllDestOrders() {
	...
	await bitz.cancelAllOpenOrders();
	await wait(1000);
}
...
async function createOrReplaceDestOrder(side, size, source_price) {
	...
	if (dest_order) {
		...
		await bitz.createAndSendCancel(dest_order.hash); // order cancelled or modified
	}
	...
	let hash = await bitz.createAndSendOrder(conf.dest_pair, side, size, dest_price);
...
async function cancelDestOrder(source_price) {
	...
	if (dest_order) {
		...
		await bitz.createAndSendCancel(dest_order.hash);
	}
	else
...
async function updateDestBids(bids) {
	...
	let dest_balances = await bitz.getBalances();
...
async function onDestDisconnect() {
	await ws_api.subscribeOrdersAndTrades(conf.dest_ws_pair);
}
...
async function start() {
	console.log('---- starting bitz-orderbook-replication');
	await source.start();
	await bitz.start();

	ws_api.on('Pushdata.order', async payload => {
		let prices = payload.map(trade => trade.p);
		let amount = await bitz.getFilledAmountByPrices(prices);
	//	let amount = await bitz.getAmountOfMyNewTrades(); // bitz lacks trade query APIs
		if (amount === 0)
